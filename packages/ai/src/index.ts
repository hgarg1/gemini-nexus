...
export const generateImageLabels = async (image: string, apiKey?: string) => {
  const genAI = new GoogleGenerativeAI(apiKey || process.env.GEMINI_API_KEY!);
...
If nothing should be saved, return {"memories":[]} with no extra text.`;
    ...
    if (!jsonMatch) return fallbackMemories(prompt);

    try {
      const parsed = JSON.parse(jsonMatch[0]);
      if (!parsed?.memories || !Array.isArray(parsed.memories)) return [];
      const extracted = parsed.memories
        .filter((m: any) => typeof m?.label === "string" && typeof m?.content === "string")
        .map((m: any) => ({
          label: m.label.trim(),
          content: m.content.trim(),
        }))
        .filter((m: any) => m.label.length >= 2 && m.content.length >= 3)
        .slice(0, 3);
...
const dedupeMemories = (items: { label: string; content: string }[]) => {
...
const sanitizeMemory = (value: string) =>
  value.replace(/\s+/g, " ").replace(/["'`]+/g, "").trim().slice(0, 160);

const fallbackMemories = (prompt: string) => {
  const memories: { label: string; content: string }[] = [];
